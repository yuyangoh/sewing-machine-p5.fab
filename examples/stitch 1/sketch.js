...
//let maxLoops = 1;
let lastLoopTime = 0;
let loopInterval = 6500; // milliseconds between loops
...
let dipDist = 4; //mm
let elevationHeight = 5; //mm
let liftHeight = 5; //mm
let x = fab.centerX;
let y = fab.centerY;
let calibrateX = 0;
let dipSpeed = 10; // mm/sec
let elevateSpeed = 30; // mm/sec
...
function fabDraw() {
  ...
  fab.moveExtrude(x, y, surfaceHeight + elevationHeight, 25, 1);
}
...
function runLoop() {
  ...
  fab.moveExtrude(x, y, surfaceHeight, dipSpeed, 0.5); // move to surface
  fab.moveExtrude(x, y, surfaceHeight + elevationHeight, elevateSpeed, 1.5); // elevate with coil
  fab.add("G4 P1000");
...
function endLoop() {
  ...
  fab.moveRetract(x, y, surfaceHeight + liftHeight, liftSpeed, 4);
